...
- 2026-05-17T (this run): Authored Phase 1 content for all five apps (d-h complete): #2 Strategy Pressure-Tester (10 classifier exemplars, 30 anchor quotes, 8 golden-set paragraphs, 2 anchor substitutions); #6 AI Eval Coverage (21 few-shot examples, 41 eval-prompt exemplars, 8 synthetic feature descriptions); #9 NSM Finder (18 NSM candidates × 6 business shapes, 10 dashboard-audit exemplars, 6 synthetic business descriptions); #24 OKR Critique (16 KR-critique examples, 6 before/after rewrites, 6 sample OKR sets, 3 anchor substitutions); #47 Mission/Vision Alignment (10 platitude-to-rewrite pairs, 4 drift-map examples, 3 MVS triples, vague-strategy fallback copy, 3 anchor substitutions); next item: Phase 2 MCP work (item i) — first pass at _open_questions in each mcp-resource.json
- 2026-05-18T00:37Z: Phase 2 MCP first pass complete (item i). Added _proposed_answers arrays to all 11 mcp-resource.json files (Phase 0: #44, #3, #53, #12-38, #51, #52; Phase 1: #2, #6, #9, #24, #47). Also ran corpus cross-check (item j): all missing anchor references are already logged substitutions; no new broken anchors found. Full backlog a-i complete; next item: owner review of _proposed_answers before locking Phase 2 specs
...
### 2026-05-18 00:37 UTC — Phase 2 MCP proposed answers (all 11 apps)
**Decision:** Added `_proposed_answers` arrays to all 11 mcp-resource.json files answering each `_open_questions` entry. Answers draw on training-data knowledge of the Jan 2026 MCP Apps spec; no live fetch was possible (no network access). Key calls: (1) `ui://` scheme confirmed valid; (2) one-time install consent recommended over per-run opt-in for all stateful tools; (3) `enum-with-null` in #12-38 outputSchema recommended for refactor to optional field; (4) alt modes (twitter-thread, podcast-cold-open) on #52 deferred to Phase 3; (5) `_debug_trace` pattern recommended for #12-38 bias visibility; (6) all Phase 1 apps lock categories/tests for Phase 2, with free-text escape-hatch fields; (7) board-prep output on #47 recommended as Markdown not PDF; (8) okr_org_coherence capped at 8 objectives.
**Why:** All answers are best-effort against spec knowledge from training. The Jan 2026 MCP Apps spec was young and not widely documented; several answers extrapolate from JSON Schema conventions and the self-contained `.mcpb` bundle model described in the spec.
**Verify with owner:** Owner should validate all proposed answers against the current MCP Apps spec before locking any spec detail for the Phase 2 build. Specifically confirm: (a) `ui://` URI pattern including bundle-ID format; (b) `requires-consent` capability flag name in the manifest; (c) whether `_`-prefixed fields are truly ignored by Jan 2026 hosts; (d) whether fs-write capability requires per-call approval or manifest-level declaration.
