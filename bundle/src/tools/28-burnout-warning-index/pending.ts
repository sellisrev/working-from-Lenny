...
export const PENDING_FILE = "burnout-index-pending.json";
export const PENDING_SCHEMA_VERSION = 1;

export type PendingBrief = z.infer<typeof BurnoutNarrateOutput>;

export interface PendingInputs {
  index: number;
  tier: string;
  override_fired: boolean;
  top_drivers: { key: string; label: string; weighted_points: number }[];
...
export async function readPending(): Promise<ReadPendingResult> {
  ...
  const briefCheck = BurnoutNarrateOutput.safeParse(parsed.data.brief);
  if (!briefCheck.success) {
    return {
      kind: "schema_mismatch",
      reason: `brief no longer matches BurnoutNarrateOutput: ${briefCheck.error.message}`,
    };
  }
  return {
    kind: "ready",
