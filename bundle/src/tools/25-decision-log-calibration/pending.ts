...
export const PENDING_FILE = "decision-log-calibration-pending.json";
export const PENDING_SCHEMA_VERSION = 1;

export type PendingBrief = z.infer<typeof DecisionLogCalibrateOutput>;
...
export async function readPending(): Promise<ReadPendingResult> {
  ...
  const briefCheck = DecisionLogCalibrateOutput.safeParse(parsed.data.brief);
  if (!briefCheck.success) {
    return {
      kind: "schema_mismatch",
      reason: `brief no longer matches DecisionLogCalibrateOutput: ${briefCheck.error.message}`,
    };
  }
  return {
    kind: "ready",
