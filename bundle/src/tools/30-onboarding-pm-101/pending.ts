...
export const PENDING_FILE = "onboarding-pm101-pending.json";
export const PENDING_SCHEMA_VERSION = 1;

export type PendingBrief = z.infer<typeof OnboardingNarrateOutput>;
...
export async function readPending(): Promise<ReadPendingResult> {
  ...
  const briefCheck = OnboardingNarrateOutput.safeParse(parsed.data.brief);
  if (!briefCheck.success) {
    return {
      kind: "schema_mismatch",
      reason: `brief no longer matches OnboardingNarrateOutput: ${briefCheck.error.message}`,
    };
  }
  return {
    kind: "ready",
