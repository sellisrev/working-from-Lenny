...
export const PENDING_FILE = "pm-non-pm-manual-pending.json";
export const PENDING_SCHEMA_VERSION = 1;

export type PendingBrief = z.infer<typeof PmNonPmNarrateOutput>;
...
export async function readPending(): Promise<ReadPendingResult> {
  ...
  const briefCheck = PmNonPmNarrateOutput.safeParse(parsed.data.brief);
  if (!briefCheck.success) {
    return {
      kind: "schema_mismatch",
      reason: `brief no longer matches PmNonPmNarrateOutput: ${briefCheck.error.message}`,
    };
  }
  return { kind: "ready", brief: briefCheck.data, saved_at: parsed.data.saved_at, inputs: parsed.data.inputs };
}
