...
export const PENDING_FILE = "saying-no-rehearsal-pending.json";
export const PENDING_SCHEMA_VERSION = 1;

export type PendingBrief = z.infer<typeof SayingNoNarrateOutput>;
...
export async function readPending(): Promise<ReadPendingResult> {
  ...
  const briefCheck = SayingNoNarrateOutput.safeParse(parsed.data.brief);
  if (!briefCheck.success) {
    return {
      kind: "schema_mismatch",
      reason: `brief no longer matches SayingNoNarrateOutput: ${briefCheck.error.message}`,
    };
  }
