...
export const PENDING_FILE = "how-you-build-pending.json";
export const PENDING_SCHEMA_VERSION = 1;

export type PendingBrief = z.infer<typeof HowYouBuildNarrateOutput>;
...
export async function readPending(): Promise<ReadPendingResult> {
  ...
  const briefCheck = HowYouBuildNarrateOutput.safeParse(parsed.data.brief);
  if (!briefCheck.success) {
    return {
      kind: "schema_mismatch",
      reason: `brief no longer matches HowYouBuildNarrateOutput: ${briefCheck.error.message}`,
    };
  }
  return {
    kind: "ready",
