...
import { DailyDoseNarrateOutput } from "../../shared/schemas";
import { dataDir } from "../../lib/paths";

export const PENDING_FILE = "daily-dose-pending.json";
export const PENDING_SCHEMA_VERSION = 1;

export type PendingBrief = z.infer<typeof DailyDoseNarrateOutput>;
...
export async function readPending(): Promise<ReadPendingResult> {
  ...
  const briefCheck = DailyDoseNarrateOutput.safeParse(parsed.data.brief);
  if (!briefCheck.success) {
    return {
      kind: "schema_mismatch",
      reason: `brief no longer matches DailyDoseNarrateOutput: ${briefCheck.error.message}`,
    };
  }
