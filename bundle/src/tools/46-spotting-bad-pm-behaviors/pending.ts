...
export const PENDING_FILE = "spotting-bad-pm-pending.json";
export const PENDING_SCHEMA_VERSION = 1;

export type PendingBrief = z.infer<typeof SpottingNarrateOutput>;
...
export async function readPending(): Promise<ReadPendingResult> {
  ...
  const briefCheck = SpottingNarrateOutput.safeParse(parsed.data.brief);
  if (!briefCheck.success) {
    return {
      kind: "schema_mismatch",
      reason: `brief no longer matches SpottingNarrateOutput: ${briefCheck.error.message}`,
    };
  }
  return {
    kind: "ready",
