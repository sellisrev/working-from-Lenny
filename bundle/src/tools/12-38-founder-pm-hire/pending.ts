...
export const PENDING_FILE = "founder-pm-hire-pending.json";
export const PENDING_SCHEMA_VERSION = 1;

export type PendingBrief = z.infer<typeof FounderNarrateOutput>;

export interface PendingInputs {
  verdict: FounderVerdict;
  playbook: FounderPlaybook | null;
  user_inputs: FounderInputsT;
  user_context: string;
}
...
export async function readPending(): Promise<ReadPendingResult> {
  ...
  const briefCheck = FounderNarrateOutput.safeParse(parsed.data.brief);
  if (!briefCheck.success) {
    return {
      kind: "schema_mismatch",
      reason: `brief no longer matches FounderNarrateOutput: ${briefCheck.error.message}`,
    };
  }
  return {
    kind: "ready",
